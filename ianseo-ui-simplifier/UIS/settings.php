...
function findModulesConfig(): ?string {
    $dir = __DIR__;
    while (true) {
        $candidate = $dir . '/config.php';
        if (is_file($candidate) && basename(dirname($candidate)) === 'Modules') {
            return $candidate;
        }
        $parent = dirname($dir);
        if ($parent === $dir) {
...
$configPath = findModulesConfig();
if (!$configPath) {
    die("❌ Impossible de trouver Modules/config.php");
}
...
function findCurrentModulePath(): ?string {
    $dir = __DIR__;
    while (true) {
        if (is_file($dir . '/UIS/settings.php')) {
            return $dir;
        }
        $parent = dirname($dir);
        if ($parent === $dir) {
...
function multiGetText(string $key): string
{
    ...
    // 2) Try custom language file: UIS/Languages/{lang}.php should define $lang[...] arrays
    $moduleDir = str_replace('\\', '/', realpath(__DIR__ . '/..'));
    $langFile  = "{$moduleDir}/UIS/Languages/{$langCode}.php";
